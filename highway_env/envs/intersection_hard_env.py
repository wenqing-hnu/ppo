...
class IntersectionhardEnv(AbstractEnv):
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
        ...
        arrive_reward = self.config["arrived_reward"] if self.has_arrived(vehicle) else 0
        time_reward = -0.1
        reward = crash_reward + arrive_reward + time_reward
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        for t in range(n_vehicles - 1):
            self._spawn_vehicle(np.linspace(0, 120, n_vehicles)[t], spawn_probability=1, go_straight=True, position_deviation=0.5, speed_deviation=0.2)
    ...
    def _spawn_vehicle(self,
        ...
        route = [1,3,3,1] # self.np_random.choice(range(4), size=2, replace=False)
        route[1] = (route[0] + 2) % 4 if go_straight else route[1]
        vehicle_type = utils.class_from_path(self.config["other_vehicles_type"])
        vehicle = vehicle_type.make_on_lane(self.road, ("o" + str(route[0]), "ir" + str(route[0]), 0),
                                            longitudinal=longitudinal + 25 + self.np_random.randn() * position_deviation,
                                            speed=8 + self.np_random.randn() * speed_deviation)

        vehicle2 = vehicle_type.make_on_lane(self.road, ("o" + str(route[2]), "ir" + str(route[2]), 0),
                                            longitudinal=longitudinal + 25 + self.np_random.randn() * position_deviation,
                                            speed=8 + self.np_random.randn() * speed_deviation)
        ...
        destination_prob = np.random.rand()
        vehicle.plan_route_to("o" + str(0))
        vehicle2.plan_route_to("o"+str(2))
        if destination_prob >= 0.2:
            vehicle.plan_route_to("o" + str(route[1]))
            vehicle2.plan_route_to("o"+str(route[3]))
...
register(
    id='intersectionhard-v0',
    entry_point='highway_env.envs:IntersectionhardEnv',
)
