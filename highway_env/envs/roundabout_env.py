...
class RoundaboutEnv(AbstractEnv):
    ...
    def _reward(self, action: int) -> float:
        ...
        crash_reward = self.config["collision_reward"] * self.vehicle.crashed
        time_reward = -0.1
        arrive_reward = 0
        if self.vehicle.lane_index[0] == Vehicle_destination:
            arrive_reward = 10
        reward = crash_reward + time_reward + arrive_reward

        return reward

    def _is_terminal(self) -> bool:
        """The episode is over when a collision occurs or when the access ramp has been passed."""
        return self.vehicle.crashed or self.steps >= self.config["duration"] or self.vehicle.lane_index[0] == Vehicle_destination
    ...
    def _make_vehicles(self) -> None:
        ...
        try:
            ego_vehicle.plan_route_to(Vehicle_destination)
